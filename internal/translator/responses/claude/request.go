...
func ConvertOpenAIResponsesRequestToClaude(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	if respClaudeAccount == "" {
		u, _ := uuid.NewRandom()
		respClaudeAccount = u.String()
	}
	if respClaudeSession == "" {
		u, _ := uuid.NewRandom()
		respClaudeSession = u.String()
	}
	if respClaudeUser == "" {
		sum := sha256.Sum256([]byte(respClaudeAccount + respClaudeSession))
		respClaudeUser = hex.EncodeToString(sum[:])
	}
	userID := fmt.Sprintf("user_%s_account_%s_session_%s", respClaudeUser, respClaudeAccount, respClaudeSession)
