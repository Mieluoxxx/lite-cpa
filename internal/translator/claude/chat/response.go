...
type ConvertAnthropicResponseToOpenAIParams struct {
	...
	ToolCallsAccumulator map[int]*ClaudeToChatToolCallAcc
}
...
// ClaudeToChatToolCallAcc holds the state for accumulating tool call data
type ClaudeToChatToolCallAcc struct {
	ID        string
	Name      string
	...
	case "message_start":
		...
		if message := root.Get("message"); message.Exists() {
			...
			if (*param).(*ConvertAnthropicResponseToOpenAIParams).ToolCallsAccumulator == nil {
				(*param).(*ConvertAnthropicResponseToOpenAIParams).ToolCallsAccumulator = make(map[int]*ClaudeToChatToolCallAcc)
			}
	...
	case "content_block_start":
		...
		if contentBlock := root.Get("content_block"); contentBlock.Exists() {
			...
			if blockType == "tool_use" {
				...
				(*param).(*ConvertAnthropicResponseToOpenAIParams).ToolCallsAccumulator[index] = &ClaudeToChatToolCallAcc{
					ID:   toolCallID,
					Name: toolName,
				}
...
func ConvertClaudeResponseToOpenAINonStream(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) []byte {
	...
	for _, line := range lines {
		if !bytes.HasPrefix(line, common.DataTag) {
			continue
		}
	...
	toolCallsAccumulator := make(map[int]*ClaudeToChatToolCallAcc)
		...
		case "content_block_start":
			...
			if contentBlock := root.Get("content_block"); contentBlock.Exists() {
				...
				if blockType == "thinking" {
					...
					index := int(root.Get("index").Int())
					toolCallsAccumulator[index] = &ClaudeToChatToolCallAcc{
