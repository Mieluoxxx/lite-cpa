...
import (
	...
	"github.com/Mieluoxxx/lite-cpa/internal/util"
...
type ConvertOpenAIResponseToAnthropicParams struct {
	...
	ToolCallsAccumulator map[int]*ChatToClaudeToolCallAcc
...
// ChatToClaudeToolCallAcc holds the state for accumulating tool call data
type ChatToClaudeToolCallAcc struct {
	ID        string
	Name      string
...
func ConvertOpenAIResponseToClaude(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if !bytes.HasPrefix(rawJSON, common.DataTag) {
		return [][]byte{}
	}
...
func convertOpenAIStreamingChunkToAnthropic(rawJSON []byte, param *ConvertOpenAIResponseToAnthropicParams) [][]byte {
	...
	if delta := root.Get("choices.0.delta"); delta.Exists() {
		if !param.MessageStarted {
			...
			results = append(results, common.AppendSSEEventBytes(nil, "message_start", messageStartJSON, 2))
		...
		if reasoning := delta.Get("reasoning_content"); reasoning.Exists() {
			for _, reasoningText := range collectOpenAIReasoningTexts(reasoning) {
				...
				if !param.ThinkingContentBlockStarted {
					...
					results = append(results, common.AppendSSEEventBytes(nil, "content_block_start", contentBlockStartJSONBytes, 2))
				...
				results = append(results, common.AppendSSEEventBytes(nil, "content_block_delta", thinkingDeltaJSONBytes, 2))
			}
		}
		...
		if content := delta.Get("content"); content.Exists() && content.String() != "" {
			...
			results = append(results, common.AppendSSEEventBytes(nil, "content_block_delta", contentDeltaJSONBytes, 2))
		...
		if toolCalls := delta.Get("tool_calls"); toolCalls.Exists() && toolCalls.IsArray() {
			if param.ToolCallsAccumulator == nil {
				param.ToolCallsAccumulator = make(map[int]*ChatToClaudeToolCallAcc)
			}
			...
			toolCalls.ForEach(func(_, toolCall gjson.Result) bool {
				...
				if _, exists := param.ToolCallsAccumulator[index]; !exists {
					param.ToolCallsAccumulator[index] = &ChatToClaudeToolCallAcc{}
				}
	...
	if finishReason := root.Get("choices.0.finish_reason"); finishReason.Exists() && finishReason.String() != "" {
		...
		if param.ThinkingContentBlockStarted {
			...
			results = append(results, common.AppendSSEEventBytes(nil, "content_block_stop", contentBlockStopJSON, 2))
		...
		if !param.ContentBlocksStopped {
			for index := range param.ToolCallsAccumulator {
				...
				if accumulator.Arguments.Len() > 0 {
					...
					results = append(results, common.AppendSSEEventBytes(nil, "content_block_delta", inputDeltaJSON, 2))
				}
	...
	if param.FinishReason != "" {
		...
		if usage.Exists() && usage.Type != gjson.Null {
			...
			results = append(results, common.AppendSSEEventBytes(nil, "message_delta", messageDeltaJSON, 2))
...
func emitMessageStopIfNeeded(param *ConvertOpenAIResponseToAnthropicParams, results *[][]byte) {
	...
	*results = append(*results, common.AppendSSEEventBytes(nil, "message_stop", []byte(`{"type":"message_stop"}`), 2))
...
func ClaudeTokenCount(ctx context.Context, count int64) []byte {
	return common.ClaudeInputTokensJSON(count)
}
