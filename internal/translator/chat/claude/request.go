...
func ConvertOpenAIRequestToClaude(modelName string, inputRawJSON []byte, stream bool) []byte {
	...
	if chatClaudeAccount == "" {
		u, _ := uuid.NewRandom()
		chatClaudeAccount = u.String()
	}
	if chatClaudeSession == "" {
		u, _ := uuid.NewRandom()
		chatClaudeSession = u.String()
	}
	if chatClaudeUser == "" {
		sum := sha256.Sum256([]byte(chatClaudeAccount + chatClaudeSession))
		chatClaudeUser = hex.EncodeToString(sum[:])
	}
	userID := fmt.Sprintf("user_%s_account_%s_session_%s", chatClaudeUser, chatClaudeAccount, chatClaudeSession)
